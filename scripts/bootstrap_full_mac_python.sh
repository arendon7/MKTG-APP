...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --target)
      ...
      ;;
    --arch)
      [[ $# -ge 2 ]] || fail "--arch requires arm64 or x86_64"
      REQUESTED_ARCH="$2"
      shift 2
      ;;
    --archive)
      [[ $# -ge 2 ]] || fail "--archive requires a local tar.gz"
      ARCHIVE_OVERRIDE="$2"
      shift 2
      ;;
    *)
...
if [[ -n "$REQUESTED_ARCH" ]]; then
  ARCH="$(normalize_arch "$REQUESTED_ARCH")" || fail "unsupported requested architecture: $REQUESTED_ARCH"
else
  ARCH="$HOST_ARCH"
fi
...
case "$ARCH" in
  arm64)
    ASSET="$FULL_MAC_PYTHON_ARM64_ASSET"
    EXPECTED_SHA256="$FULL_MAC_PYTHON_ARM64_SHA256"
    URL="$FULL_MAC_PYTHON_ARM64_URL"
    ;;
  x86_64)
    ASSET="$FULL_MAC_PYTHON_X86_64_ASSET"
    EXPECTED_SHA256="$FULL_MAC_PYTHON_X86_64_SHA256"
    URL="$FULL_MAC_PYTHON_X86_64_URL"
    ;;
  *)
...
if [[ -x "$EXTRACT/python/bin/python3" ]]; then
  SOURCE_RUNTIME="$EXTRACT/python"
elif [[ -x "$EXTRACT/bin/python3" ]]; then
  SOURCE_RUNTIME="$EXTRACT"
else
  fail "verified runtime archive does not contain executable bin/python3"
fi
...
cat > "$TARGET/FULL_MAC_PYTHON_RUNTIME.json" <<JSON
{
  ...
  "schema": "binario.marketing.full-mac-python-runtime.v1",
  "source_asset": "$ASSET",
...
printf '\nFULL MAC PYTHON BOOTSTRAP: PASS\n'
printf 'Runtime: %s\n' "$TARGET"
printf 'Python: %s\n' "$FULL_MAC_PYTHON_VERSION"
